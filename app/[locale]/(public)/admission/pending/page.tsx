...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  assertLocale(locale);
  const t = await getTranslations({ locale, namespace: 'admission.pending' });
  return { title: t('pageTitle') };
}

export default async function AdmissionPendingPage({ params }: Props) {
  const { locale } = await params;
  assertLocale(locale);
  setRequestLocale(locale);

  const t = await getTranslations('admission.pending');
