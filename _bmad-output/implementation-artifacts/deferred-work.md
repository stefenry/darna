...
- **Présence live « un autre co_mod consulte ce signalement »** [`app/[locale]/comod/moderation/[id]`] — Story 5.3 AC concurrence : la moitié « notice quand un autre co_mod consulte EN CE MOMENT » nécessite Realtime/heartbeat (hors stack MVP « polling-à-l'ouverture »). Implémenté : anti double-modération (garde RPC `state='open'` → `already_resolved`) + affichage de la résolution si déjà traité. La présence live temps-réel est différée (Realtime Supabase / Epic 7).
- **SLA modération fuseau Africa/Casablanca** [`lib/moderation/sla.ts`] — `presenceHoursElapsed` calcule la fenêtre 7h-23h en UTC (déterministe/testable). Le décalage Casablanca (UTC+1, ±DST Ramadan) ≤ 1h ne change pas la surface utile (signaler les retards) ; raffinement tz différé.
- **Notif modération auteur/reporter en AR** [`lib/email/templates/content-removed-author`, `report-kept-reporter`] — Envoi FR-only au MVP (templates AR = fallback FR). Locale par profil + traduction AR RTL en V1.5 (comme magic-link/admission).
- **Dossier escalade juridique sur R2** [`lib/moderation/legal-storage.ts`] — Story 5.5 stocke le dossier sur Supabase Storage (URL signée 30j) car R2 (AR29) n'est pas provisionné au MVP (env `R2_*` optionnels, pas de client S3). Quand R2 sera provisionné : remplacer `uploadDossier` par un upload R2 + presign 30j (S3 SDK). Fallback inline e-mail conservé.
- **Contact juridique multi-résidence** [`escalateReportLegal`] — `LEGAL_CONTACT_EMAIL` est unique (env global), MVP mono-résidence. V1.x : contact juridique par résidence.

## Deferred from: code review of Epic 5 (review fixes `8e34da3`) (2026-06-21)

Revue adversariale (Blind Hunter + Edge Case Hunter + Acceptance Auditor) du commit de revue Epic 5. Les 12 correctifs sont AC-conformes ; items ci-dessous = durcissements mineurs / pré-existants, non bloquants.

- **[5.5] Escalade — impasse en cas de double échec** [`app/[locale]/comod/moderation/actions.ts`] — Si l'e-mail Brevo échoue (soft-fail) ET l'upload dossier renvoie `null`, le co_mod a un warning sans lien dossier ; l'escalade est committée (`pending_legal`) sans recours UI. Probabilité faible (deux effets de bord échouent). Suivi : action « régénérer dossier / renvoyer e-mail » sur un report `pending_legal`.
- **[5.5] Scrub PII — réglage du seuil** [`lib/moderation/dossier.ts`] — Tension under/over-redaction : numéro < 8 chiffres passe ; `user@host` sans TLD non masqué ; à l'inverse une réf ≥ 8 chiffres (IBAN/montant) citée est masquée → peut dégrader la valeur probante. Couche de défense en profondeur sur texte libre. Suivi : affiner les regex selon retours terrain.
- **[5.2] Self-report non gardé côté serveur** [`app/actions/report-submit.ts`] — Bouton « Signaler » masqué sur son contenu (UI) mais l'action serveur ne rejette pas un signalement de sa propre cible (requête forgée). Bénin (report inutile traité par co_mod). Pré-existant. Suivi : garde serveur `target.author != reporter`.
- **[5.1] Policy RLS orpheline après lockdown** [`supabase/migrations/20260705090000_review_5_moderation_log_lockdown.sql`] — `moderation_log_consent_residence_select` survit ; morte (grant SELECT révoqué) donc inoffensive. Nettoyage cosmétique : la `drop` pour cohérence.
- **[Epic 2, pré-existant] Tri commentaires artisan sans tie-breaker** [`app/[locale]/community/artisan/[slug]/data.ts`] — `order created_at desc limit 10` sans `id` secondaire → non-déterminisme aux horodatages égaux (même défaut que la pagination journal corrigée en 5.4, hors périmètre Epic 5). Suivi : ajouter `.order('id', desc)`.
