...
const ACTION_CLASS =
  'inline-flex min-h-touch items-center rounded bg-bg-card px-4 text-sm font-medium text-link hover:bg-neutral-300';

export function ContributorPanel({ locale, slug }: { locale: string; slug: string }) {
  const t = useTranslations('community.artisan.owner');
  return (
    <section aria-label={t('title')} className="flex flex-col gap-2 rounded bg-bg-soft p-4">
