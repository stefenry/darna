...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  assertLocale(locale);
  const t = await getTranslations({ locale, namespace: 'admission.refused' });
  return { title: t('pageTitle') };
}

export default async function AdmissionRefusedPage({ params }: Props) {
  const { locale } = await params;
  assertLocale(locale);
  setRequestLocale(locale);

  const t = await getTranslations('admission.refused');
